...
% kba: "cut the head off" is a bit jarring IMHO
% kba: Continuing training of an existing model is pretty tricky in
% my (support) experience. Rui, can you say some more on how well
% works with different engines? I.e. is it (currently) a viable approach to
% start with the default model and expand on that with new data?
...
% kba: We plugged it before, one plug per paper for ocrd-train seems plenty...
...
specifying the OCR engine(s) to use and the parameters. The user
...
user can check the job progress. After the training of a model is finished, it can be
downloaded. The user can also submit an evaluation
request with an evaluation dataset and a selected model and receive TODO.
...
executes the work outside an HTTP request-response cycle.  It would put job
