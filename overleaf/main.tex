...
% CN: The release date is very misleading here, LSTM-RNN tesseract has been around (and in use) for much longer, usually one claims Ray Smith's DAS2016 tutorial as the "release": https://github.com/tesseract-ocr/docs/tree/master/das_tutorial2016
...
% CN: Before we conclude, I would expect another section with some minimal evaluation/results discussion, e.g. was it (not?) possible to a) train models for all four engines and b) using the same data according to specs with okralact and c) how do these models compare to ones that are shipped with each engine? 
